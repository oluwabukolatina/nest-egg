...
describe('Loan Application Integration Tests', () => {
    ...
    describe('GET /loan-applications/:id', () => {
        test('should get a loan application successfully', async () => {
            ...
            const response = await request(app)
                .get(`${FIND_LOAN_APPLICATION_URL.replace(':id', createApplication.body.data.data.id)}`)
            expect(response.status).toBe( 200)
            expect(response.body).toHaveProperty('data');
            expect(response.body.data).toHaveProperty('loanApplication');
            expect(response.body.data.loanApplication).toHaveProperty('id', createApplication.body.data.data.id);
            ...
            expect(response.body.data.loanApplication).toHaveProperty('annual_interest_rate',);
            expect(response.body.data.loanApplication).toHaveProperty('monthly_payment',);
            expect(response.body.data.loanApplication).toHaveProperty('status','PENDING');
        ...
        test('should return 404 when loan application is not found', async () => {
            ...
            const response = await request(app)
                .get(`${FIND_LOAN_APPLICATION_URL.replace(':id', "9876567")}`)
            expect(response.status).toBe( 404)
            expect(response.body).toHaveProperty('message', 'Loan Application Not Found');
        });
    });
